...
def perform_bayes_discriminant_analysis(
    ...
    - prior_probabilities: 先验概率列表（可选，默认使用等概率或样本比例）
    ...
    try:
        ...
        if prior_probabilities is not None:
            if len(prior_probabilities) != n_groups:
                raise ValueError("先验概率数量必须与组数相同")
            priors = np.array(prior_probabilities)
            # 验证概率之和为1
            if not np.isclose(np.sum(priors), 1.0):
                raise ValueError("先验概率之和必须为1")
        else:
            # 默认使用等概率
            priors = np.ones(n_groups) / n_groups
        ...
        for i, cov in enumerate(group_covs):
            try:
                ...
                raise ValueError(f"第{i+1}组协方差矩阵奇异，无法计算贝叶斯判别函数")
        
        # 对测试样本进行贝叶斯判别（如果提供了测试数据）
        ...
        if test_data is not None:
            for i, test_sample in enumerate(test_samples):
                ...
                for j in range(n_groups):
                    ...
                    # 计算判别函数值（对数后验概率）
                    # 忽略常数项 -0.5*n_vars*log(2*pi)
                    discriminant_score = -0.5 * mahalanobis_sq - 0.5 * group_log_dets[j] + np.log(priors[j])
                    discriminant_scores.append(discriminant_score)
                    
                    # 计算似然函数值
                    likelihood = np.exp(-0.5 * mahalanobis_sq) / np.sqrt(np.linalg.det(group_covs[j]))
                    posterior = likelihood * priors[j]
                    posteriors.append(posterior)
                
                # 归一化后验概率
                total_posterior = sum(posteriors)
                normalized_posteriors = [p / total_posterior for p in posteriors] if total_posterior > 0 else [1/n_groups] * n_groups
                
                # 选择后验概率最大的组作为分类结果
                best_group_idx = np.argmax(discriminant_scores)
                test_classifications.append({
                    "sample_index": i+1,
                    "classified_group": group_names[best_group_idx],
                    "posterior_probabilities": dict(zip(group_names, normalized_posteriors)),
                    "discriminant_scores": dict(zip(group_names, discriminant_scores))
                })
        ...
        # 绘制先验概率和后验概率对比（如果有测试数据）
        if test_data is not None and len(test_classifications) > 0:
            plt.subplot(1, 2, 1)
            # 绘制第一个测试样本的后验概率
            first_sample = test_classifications[0]
            posterior_probs = [first_sample["posterior_probabilities"][name] for name in group_names]
            prior_probs = priors.tolist()
            
            x = np.arange(n_groups)
            width = 0.35
            plt.bar(x - width/2, prior_probs, width, label='先验概率')
            plt.bar(x + width/2, posterior_probs, width, label='后验概率（样本1）')
            
            plt.xlabel('组别')
            plt.ylabel('概率')
            plt.title('先验概率与后验概率对比')
        ...
        # 保存贝叶斯判别分析图
        plot_filename = f"bayes_discriminant_analysis_{uuid.uuid4().hex}.png"
        ...
        result = {
            ...
            "prior_probabilities": dict(zip(group_names, priors.tolist())),
            "group_means": {group_names[i]: group_means[i].tolist() for i in range(n_groups)},
            "mahalanobis_distances": mahalanobis_distances,
            "bayes_plot_url": plot_url,
            "interpretation": _get_interpretation(mahalanobis_distances, priors)
        }
        ...
        # 让异常向上抛，FastMCP 会处理
        raise RuntimeError(f"贝叶斯判别分析计算失败: {str(e)}") from e


def _get_interpretation(mahalanobis_distances: List[Dict], priors: np.ndarray) -> str:
    """
    根据马氏距离和先验概率提供结果解释
    """
    ...
    interpretation = f"贝叶斯判别分析结果：\n"
    interpretation += f"先验概率: {dict(zip(range(len(priors)), priors))}\n"
    ...
    interpretation += "\n该工具使用贝叶斯判别法，基于贝叶斯定理和多元正态分布假设进行分类。"
    interpretation += "通过计算后验概率来进行判别，可以考虑先验信息，是一种理论上最优的分类方法。"
